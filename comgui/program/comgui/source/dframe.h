...
class dWidget {
	protected:
		...
		int		m_id;		//用于响应消息的id,设定了父控件的情况下下会确保id唯一
	...
	public:
		...
		/***基本操作函数***/ 
		virtual bool setId( int );	//重新设置Id,Id不能为空,不能重复,有必要的话,调用重载的函数 
		int	getId( void ) const;			//获取Id
...
class dFrame : public dField {
	private:
		...
		chain<dEvent>	m_ev_list;	//向用户传递的控件事件,当事件列表被更新时会向窗口传递 
		/***frame类静态属性***/
		static chain<dFrame*> global_frame_list;	//全局frame列表,用于从任何地方找到所有frame
	...
	public:
		...
		//Id相关
		bool	setId( int );				//重载的会在frame链中进行查重
		//获取客户区位置及形状
		dRect	getClientRect( void );		//子控件坐标的坐标原点是客户区左上角
		//获取客户区设备描述表句柄
		HDC		getHDC( void );
		/***功能性函数***/
		static dFrame* findFrame( int );	//通过id寻找frame 
