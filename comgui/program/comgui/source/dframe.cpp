...
int	dWidget::getId( void )	const						//获取Id
{
	return this->m_id;
}
...
dWidget*	dWidget::searchFrame( int id )
{
...
void	dField::setRender( dRender render )			//设置绘制器 
{
	this->m_render = render;
}
dRender	dField::getRender( void )					//获取绘制器 
{
	return this->m_render;
}
/***有关子控件的操作函数***/
dWidget*	dField::searchChild( int id )
{
	dWidget* tag;
	dField* tcd;
...
bool		dField::removeChild( int id )	//将子节点直接删除 
{
...
//Id相关
bool	dFrame::setId( int id )		//重载的会在frame链中进行查重
{
...
/***功能性函数***/
dFrame* dFrame::findFrame( int id )	//通过id寻找frame 
{
	dFrame* tag = 0;
...
chain<dFrame*> dFrame::global_frame_list = chain<dFrame*>();
//end of file;	
}
#endif
